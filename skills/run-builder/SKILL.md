...
**Amendment 2026-09-18 (owner directive, CAO pass; "All work lanes must be able to be launched from the hub folder
in VS Code"):** the launch standard for EVERY lane — builder, gate, marathon, watcher — is a VS Code extension chat
opened on the Hub folder (`D:\Hub` on HQ, `C:\Hub` on ENGINE/FIELD). Twice a lane stopped and told the owner to open
a worktree folder or run a PowerShell launcher; that is a defect in the prompt, not an instruction to the owner.
Consequences, all built: (1) `PROMPT-GATE.md.tmpl` and `PROMPT-BUILDER.md.tmpl` open with the standard and reach
their trees by an explicit `cd` inside each command, never by reopening the chat; (2) the gate's first command is
`python <claude home>/hooks/no_test_run_guard.py --claim-seat {REPO_PATH}` — the guard hook, which alone knows the
session id, binds `{REPO_PATH}\.walkaway\GATE_SEAT` to that chat (`session=`), and the gate's T1 is
`cd {REPO_PATH}; scratch-run.ps1 …`, which resolves the bound seat from the Hub root; a builder never runs the claim,
a second session cannot take a bound seat, a subagent's claim is refused (drilled: 8/8, guard self-test 0 failures);
(3) `new-run.ps1`'s launch card says "open VS Code on <hub root>; paste each prompt into its own chat there"; (4)
`walkaway.ps1`, `FLEET_GATE_SEAT=1`, the integrated terminal and any per-worktree VS Code window are OPTIONAL
conveniences and never a stop condition — a prompt that makes any of them a precondition is wrong and is fixed, not
obeyed. No prompt may render "open the chat on <tree>" or "run walkaway.ps1 first". Charter amendment of the same
date (Article 1, 8, 31 appended section) is the law; this skill is one of its instruments.
